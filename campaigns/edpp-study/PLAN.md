...
Last updated 2026-07-29. Supersedes the ordering in `TODO.md`, which predates the
...
Committed at `infocom` `b036b5f`, local only, NOT pushed to Overleaf. Replaced
...
**3. Ladder below `always`'s ceiling.** THE outstanding methodology gap. Every
measured `always` goodput of 0.000 sits at a load above 8.31 or 4.15 req/s, where
`always` is saturated by construction, so it is a tautology and not a comparison.
The band below its ceiling has never been sampled on either prefill cell.

Design: rates as fractions of the **measured** ceilings (19.75 and 11.64), with at
least one point below `always`'s own ceiling. Arms are `always`, dpVaR, and the
forced plan at f\* as a static-optimal reference. No `never` in any form.

Suggested rates: lean 6 / 8 / 14 / 20; bound 3.5 / 4.0 / 8.2 / 11.8.

**4. Style pass on Problem Formulation.** Real violations of
...
**5. Then, and only then, Policy Evaluation.** Everything in it rests on the
...
- **Overleaf.** `infocom` tracks `overleaf/main`. Two local commits are unpushed.
  A push surfaces for a co-author.
