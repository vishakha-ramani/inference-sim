#!/usr/bin/env python3
"""Emit a total --pd-plan CSV realising a fixed mixing fraction f on 1P2D.

What f means
------------
f is the fraction of requests that are DISAGGREGATED: their prefill runs on
instance 0 (the prefill-only instance) and their decode on a mixed instance. The
remaining 1 - f are served whole on a mixed instance, which the plan expresses
...
So f = 1 reproduces --pd-decider always and f = 0 reproduces --pd-decider never.
...
ratio allows, which means every prefix of the trace already sits at fraction f.
Sampling f from an RNG would give the same mean with added burstiness and would
...
Usage
-----
    python3 campaigns/edpp-study/make_pd_plan.py --n 9600 --f 0.387 > plan.csv
    python3 campaigns/edpp-study/make_pd_plan.py --n 100 --f 0 --decode-instances 2
"""
...
def main():
    ...
    ap.add_argument("--f", type=float, required=True,
                    help="fraction of requests disaggregated (prefill on the prefill instance)")
    ...
    if not 0.0 <= args.f <= 1.0:
        raise SystemExit(f"--f must be in [0, 1], got {args.f}")
    ...
    # error crosses one. Over n rows this places exactly round(f*n) of them, as
