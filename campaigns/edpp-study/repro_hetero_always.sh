...
D=campaigns/edpp-study/out/hetero_fair; OUT="$D/out"; mkdir -p "$OUT"
CAP=256; ARRIVAL_S="${ARRIVAL_S:-300}"; SEEDS="${SEEDS:-42}"
T=80.5; I=42.74; E=2783.3      # v3-derived hetero targets
...
for R in 70 90 110; do
  for s in $SEEDS; do
    ...
    M="$OUT/${R}_always_${s}.json"
    ./blis run --model "$MODEL" --workload-spec "$W" --num-instances 3 \
      --prefill-instances 1 --decode-instances 2 --decode-routing-scorers "queue-depth:1" --max-num-running-reqs "$CAP" \
      --policy-config "$D/bundle.yaml" "${SLO[@]}" --pd-decider always \
