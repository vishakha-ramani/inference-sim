...
- **Fallback:** when the decode sub-request never recorded its own metrics (drop-at-transfer-start / late-drop / timed-out-while-queued, #1511), fall back to the `parent.CompletionTime − ArrivalTime` value (no regression). INV-5 on this fallback branch is **not** guaranteed by construction — it inherits the pre-#1513 behavior (e.g. a decode sub-request that emits a first token and then times out takes the TTFT primary path but the E2E fallback, so a deadline inside the post-first-token OTPT window can leave TTFT marginally above E2E). Tracked with the other drop/timeout edge cases in #1511.
